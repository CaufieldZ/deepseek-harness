...
Product plugins that add model-visible request context without defining a tool. `agent-instructions` is included by the default `dsh-agent-spine-demo` bundle and can be disabled through bundle config; `time-context`, `tmux-context`, `vscode-context`, `session-reference`, `file-reference`, and `file-reference-local` are opt-in.
...
| [`vscode-context/`](vscode-context/README.md) | VS Code editor-state context fed over a state file | — |
